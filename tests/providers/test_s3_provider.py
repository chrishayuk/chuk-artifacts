...
class TestS3ProviderIntegration:
    """Integration tests for S3 provider (requires real S3/MinIO)."""

    @pytest.fixture
    def real_s3_factory(self):
        """Create a real S3 factory for integration tests."""
    ...
    @pytest.mark.asyncio
    async def test_real_s3_roundtrip(self, real_s3_factory):
        """Test complete roundtrip with real S3."""
        bucket = os.getenv("ARTIFACT_BUCKET", "chuk-sandbox-2")
        key = "pytest/integration-test.txt"
        test_data = b"Integration test data"
        
        try:
            async with real_s3_factory() as s3:
                # Put object
                ...
                assert get_response["Metadata"]["test"] == "integration"
            ...
            if "NoSuchBucket" in str(e):
                pytest.skip(f"Bucket {bucket} not available for integration tests")
            else:
                raise

    @pytest.mark.asyncio
    async def test_real_s3_presigned_url(self, real_s3_factory):
        """Test presigned URL generation with real S3."""
        bucket = os.getenv("ARTIFACT_BUCKET", "chuk-sandbox-2")
        key = "pytest/presigned-test.txt"
        ...
        try:
            ...
            if "NoSuchBucket" in str(e) or "oauth" in str(e).lower():
                pytest.skip("S3 service limitations for integration tests")
            else:
                raise


# Test configuration and markers
# Note: Only async tests get the asyncio mark, sync tests are unmarked
