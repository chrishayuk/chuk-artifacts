...
async def secure_session_demo():
    ...
    try:
        ...
        print("✅ Session isolation maintained perfectly!")
...
async def security_validation_demo():
    ...
    store = ArtifactStore(
        storage_provider="filesystem", session_provider="memory", bucket="security-demo"
    )
    ...
    try:
        ...
        print("🏢 Testing with multiple company sessions:")
        print(f"   - {session_a}")
        print(f"   - {session_b}")
        print(f"   - {session_c}")
        ...
        security_tests = [
            ...
            (
                "Overwrite A from B",
                lambda: store.write_file(
                    "hacked",
                    filename="hack.txt",
                    session_id=session_b,
                    overwrite_artifact_id=file_a,
                ),
            ),
        ]
