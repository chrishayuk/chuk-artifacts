...
async def clean_grid_demo():
    ...
    try:
        async with ArtifactStore(
            ...
            print("   Cross-sandbox discovery: prefix search")
            print("   Session-based security: strict isolation")
            print("")
            print("✅ Simple operations:")
            print("   Mandatory sessions (no anonymous artifacts)")
            print("   Auto-allocation when needed")
            print("   Grid-aware key generation")
            ...
            copied_file = await store.copy_file(files[0], new_filename="bob_copy.txt")
            print(f"✅ Same-session copy allowed: {copied_file}")

            print("\n🎉 Clean grid architecture demo completed!")
            print("🏗️  Simple, focused, federation-ready design")
...
async def multi_sandbox_demo():
    ...
    try:
        ...
        async with ArtifactStore(bucket="sandbox-a", sandbox_id="company-a") as store_a:
            os.environ["ARTIFACT_FS_ROOT"] = temp_dir2
            async with ArtifactStore(
                bucket="sandbox-b", sandbox_id="company-b"
            ) as store_b:
...
async def main():
    ...
    print("✅ Mandatory sessions")
    print("✅ Grid-only paths")
    print("✅ Federation ready")
    print("✅ Simple & focused")
