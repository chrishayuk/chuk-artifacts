...
TEST_CONFIGS = [
    (
        "memory",
        "filesystem",
        "Memory sessions + filesystem storage (recommended for testing)",
    ),
...
async def test_store_configuration(
    ...
    try:
        ...
        store = ArtifactStore(bucket=bucket_name, sandbox_id="smoke-test")
        ...
        if session_ok and storage_ok:
            ...
            results.failure(
                "Configuration validation failed", Exception(str(config_status))
            )
    ...
    try:
        ...
        if storage_provider == "s3":
            ...
            if any(
                ...
                for url in [short_url, medium_url, long_url]
            ):
                results.success("S3 presigned URL format correct")
            else:
                results.failure("Unexpected S3 URL format")
        ...
        if "NotImplementedError" in str(type(e)) or "oauth" in str(e).lower():
            results.success(
                "Presigned URLs correctly unavailable for this credential type"
            )
        elif "Object not found" in str(e) and storage_provider == "memory":
            results.success(
                "Memory provider isolation prevents presigned URLs (known limitation)"
            )
        else:
...
async def test_batch_operations(store: ArtifactStore, results: TestResult):
    ...
    try:
        ...
        artifact_ids = await store.store_batch(
            batch_items, session_id="batch_test_session"
        )
        ...
        if valid_ids:
            try:
                ...
                if "NoSuchKey" in str(e) and "memory" in str(
...
async def test_file_operations(store: ArtifactStore, results: TestResult):
    ...
    try:
        ...
        copy_id = await store.copy_file(
            file_id, new_filename="test_files/copy_test.txt", new_meta={"copied": True}
        )
        ...
        if "NoSuchKey" in str(e) and "memory" in str(store._storage_provider_name):
            ...
            results.failure("File operations failed", e)
