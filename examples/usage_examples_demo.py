...
async def basic_usage():
    ...
    store = ArtifactStore(
        storage_provider="memory", session_provider="memory", bucket="basic-test"
    )
...
async def filesystem_usage():
    ...
    store = ArtifactStore(
        storage_provider="filesystem", session_provider="memory", bucket="fs-test"
    )
...
async def multi_store_usage():
    ...
    memory_store = ArtifactStore(
        storage_provider="memory", session_provider="memory", bucket="memory-bucket"
    )

    fs_store = ArtifactStore(
        storage_provider="filesystem", session_provider="memory", bucket="fs-bucket"
    )

    try:
        # Use memory store
        memory_id = await memory_store.store(
            b"memory data", mime="text/plain", summary="Memory artifact"
        )
        print(f"✅ Memory store: {memory_id}")

        # Use filesystem store
        fs_id = await fs_store.store(
            b"filesystem data", mime="text/plain", summary="Filesystem artifact"
        )
...
async def batch_usage():
    ...
    store = ArtifactStore(
        storage_provider="filesystem", session_provider="memory", bucket="batch-test"
    )
...
async def robust_usage():
    ...
    store = ArtifactStore(
        storage_provider="filesystem", session_provider="memory", bucket="robust-test"
    )
...
async def web_framework_example():
    ...
    store = ArtifactStore(
        storage_provider="filesystem", session_provider="memory", bucket="web-uploads"
    )
...
async def development_pattern():
    ...
    store = ArtifactStore(
        storage_provider="memory", session_provider="memory", bucket="dev-test"
    )

    try:
        stats = await store.get_stats()
        print(
            f"✅ Development setup: {stats['storage_provider']}/{stats['session_provider']}"
        )

        artifact_id = await store.store(
            b"Development test data", mime="text/plain", summary="Development artifact"
        )
...
async def staging_pattern():
    ...
    try:
        ...
        stats = await store.get_stats()
        print(
            f"✅ Staging setup: {stats['storage_provider']}/{stats['session_provider']}"
        )

        artifact_id = await store.store(
            b"Staging test data", mime="text/plain", summary="Staging artifact"
        )
...
async def run_all_examples():
    ...
    print("🏗️ Modular architecture working perfectly!")
    print("ℹ️  Memory provider isolation is expected behavior")
