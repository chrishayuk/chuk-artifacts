...
"""
        ...
        config_encoded = base64.b64encode(json.dumps(config_content, indent=2).encode()).decode()
        ...
        fake_image = b'\x89PNG\r\n\x1a\n' + b'fake image data' * 100  # Fake PNG header + data
...
"""
        ...
        updated_config_id = await store.write_file(
            ...
            meta={"updated_by": "mcp_client", "version": "1.1.0", "replaces": config_id}
        )
        ...
        for directory, info in sorted(directories.items()):
            print(f"     📁 {directory}: {len(info['files'])} files, {info['bytes']:,} bytes")
        ...
        print(f"🔐 Other session '{other_session}': {len(other_files)} files (isolated)")
...
"""
        ...
        print(f"✅ Each user can only see their own files")
        print(f"✅ No cross-session data leakage")
...
async def mcp_performance_demo():
    ...
    try:
        ...
        print(f"✅ Created 20 files in {create_time:.3f}s ({20/create_time:.1f} files/sec)")
        ...
        print(f"✅ Listed uploads/ directory ({len(upload_files)} files) in {dir_time:.3f}s")
        ...
        print(f"✅ Read {read_count} files in {read_time:.3f}s ({read_count/read_time:.1f} reads/sec)")
        ...
        print(f"✅ Copied {copy_count} files in {copy_time:.3f}s ({copy_count/copy_time:.1f} copies/sec)")
